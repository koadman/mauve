...
public class DCJ
{
	...
	/********************************************
	 ...
	 holds Vertices left,right, value of genomeA and value of genome B
	 performs traversing along black lines to print state of genome A
	 ********************************************/
	class SB
	{
		...
		public void setLeft(Vert v){this.left=v;}
		public void setRight(Vert v){this.right=v;}
		public void setIdentity(String s){this.identity=s;}
		public void turnValue(int s){
			this.valueA=s;
		//{try{this.valueA=Integer.parseInt(s);}catch(Exception e){System.out.println("format error\n"+e);System.exit(0);}		
		}
		public void setValueA(int i){this.valueA=i;}
		public void setValueB(int i){this.valueB=i;}
		////newly constructed vertices need to be set to point to this SB.
		public boolean setVertPointers()
		{
			if(this.getLeft()==null||this.getRight()==null){return false;}
			else{this.getLeft().setSB(this);this.getRight().setSB(this);}
			return true;
		}

		////retrieval methods
		public boolean wasVisited(){return this.visited;}
		public Vert getLeft(){return this.left;}
		public Vert getRight(){return this.right;}
		...
		////polarity of the SB, pos value head is right Vert, neg value head is left Vert.
		public Vert getHeadA()
		{
			if(this.getValueA()<0){return this.getLeft();}
			else{return this.getRight();}
		}
		public Vert getTailA()
		{
			if(this.getValueA()<0){return this.getRight();}
			else{return this.getLeft();}
		}
		public Vert getHeadB()
		{
			//if(this.getValueB()<0){return this.getLeft();}else{return this.getRight();}
			if(this.getValueB()==this.getValueA()){return this.getRight();}
			else{return this.getLeft();}
		}
		public Vert getTailB()
		{
			//if(this.getValueB()<0){return this.getRight();}	else{return this.getLeft();}
			if(this.getValueB()==this.getValueA()){return this.getLeft();}
			else{return this.getRight();}
		}
	...
	/********************************************
	 Class Vert for Vertices
	 Every SB has a left and right Vert
	 Vert contains Variables black and grey which
	 point to other Verts
	 ********************************************/
	class Vert
	{
		...
		{
			this.black=b;this.grey=g;this.gene=s;sb=null;chromosome="";isCap="";seenCap=false;}
		public Vert(String s)
		{
			this.black=null;this.grey=null;this.gene=s;sb=null;chromosome="";isCap="";seenCap=false;}
		//MUTATOR methods
		public void setBlack(Vert v){this.black=v;}
		public void setGrey(Vert v){this.grey=v;}
		public void setSB(SB s){this.sb=s;}
		public void setGene(String s){this.gene=s;}
		//name the chromosome the vertex belongs to
		public void setChromosome(String s){this.chromosome=s;}
		//name the cap 
		public void setIsCap(String b){this.isCap=b;}
		//cap a vertex, true is A cap(black line), false is B cap (grey line)
		public void setSeen(boolean b){this.seenCap=b;}
		...
		public Vert cap(boolean b)
		{
			...
			if(b)
			{
				newcap=new Vert("A");newcap.setIsCap(newcap.getGene());newcap.setSB(new SB());newcap.setBlack(newcap);newcap.setGrey(newcap);
				this.joinBlack(newcap);
				newcap.setGene(""+newcap.getGene()+(++counta));
			}
			else
			{
				newcap=new Vert("B");newcap.setIsCap(newcap.getGene());newcap.setSB(new SB());newcap.setBlack(newcap);newcap.setGrey(newcap);
		...
		public Vert getBlack(){return this.black;}
		public Vert getGrey(){return this.grey;}
		public SB getSB(){return this.sb;}
		public String getCap(){return this.isCap;}
		public boolean isCap(){return ((this.getCap()).length()>0);}
		public String getChromosome(){return this.chromosome;}
		public boolean seen(){return this.seenCap;}

		//print methods
		//prints the main variables of Vert, try-catch for events where vars have not been initialized.
		public void printMe()
		{
			buf.append("Block: ");
			try{buf.append(this.getGene());}
			catch(Exception e){}
			buf.append("	black:");
		
			try{buf.append(this.getBlack().getGene());}
			catch(Exception e){}
			buf.append("	Grey: ");
		
			try{buf.append(this.getGrey().getGene());}
			catch(Exception e){}
			buf.append("	SB: ");
		
			try{buf.append(this.getSB().getIdentity());}
			catch(Exception e){}
			if(this.isCap()) {buf.append("	cap:"+this.getCap());}
			buf.append("\n");
		}

		//traverse capped genome with alternating black/grey paths (for closing paths to generate HP cycle) and returns the end cap
		public Vert traversePath(boolean b)
		{
			if(this.isCap()) return this;
			if (b) return this.getBlack().traversePath(false);
		...
		public Vert traversePath(boolean b,Vector v)
		{
			...
			if(!b) {v.addElement(this);}
			if(this.isCap()) return this;
			if (b) return this.getBlack().traversePath(false,v);
			return this.getGrey().traversePath(true,v);
		}

		//this = a = cap, b is another cap. closepath forms a cycle, the method depends on the Cap values (A&B A&A or B&B)
		public void closePath(Vert b)
		{
			//a and b are both B caps
			if ( (this.getCap().equals(b.getCap()))&&(this.getCap().equals("B"))){this.joinBlack(b);}
			//a and b are both A caps
			if ((this.getCap().equals(b.getCap()))&&(this.getCap().equals("A"))){this.joinGrey(b);}
			//a!=b and a is an A cap (black line out)
			if (!(this.getCap().equals(b.getCap()))&&this.getCap().equals("A")){this.setGrey(b.getGrey());(b.getGrey()).setGrey(this);}
			//a!=b and a is a B cap (grey line out) - is reverse of above code
			if (!(this.getCap().equals(b.getCap()))&&this.getCap().equals("B")){b.setGrey(this.getGrey());(this.getGrey()).setGrey(b);}
		}//end closepath
		...
		//tests if the Vert  exists as a 1-cycle. Which is characterized as a black and grey line pointing to the same Gene
	...
	/**********************************************
	 Class DCJ
	 Loads the data and parses
	 Creates and populates Vectors for SBs and grey lines
	 in the process.
	 *********************************************/
	public int counta=0;
	public int countb=0;
	...
	public DCJ(StringTokenizer A, StringTokenizer B,String species1,String species2)
	{
		buf=new StringBuffer(bufsize);
		opBuf=new StringBuffer();
		speciesA=species1;
	...
	{
		Vert temp;
		if(!suppress) buf.append(greyVector.size()+ " Grey lines\n");
		this.printSBBlack();
		while(greyVector.size()>0)
		{
			...
			if(this.DCJ(temp))
			{
				if(!suppress){this.printSBBlack();}
			}
		}//end while
		if(suppress) buf.append(countDCJ+"	\n");
	}

	public String getLog(){return this.buf.toString();}
	public String getOpBuf(){return this.opBuf.toString();}
	...
	public int getCount() {return countDCJ;}
	...
	public boolean DCJ(Vert v)
	{
		...
		if (v.isOneCycle())
		{
			//code for removing 1-cycle from Vector of lines, this 1-cycle is formed from a previous call that was a 2-cycle forked into 2 1-cycles, it is not counted as a DCJ call
			//remove(object) from a vector is O(n) time however since the object is always the first object, it's constant.
		...
		else
		{
			...
			++countDCJ;
			if(!suppress){tempbuf.append(countDCJ+" JOIN: "+v.getBlack().getBlack().getGene() + " to "+v.getGrey().getBlack().getBlack().getGene() +"	"); }
			v.getBlack().joinBlack(v.getGrey().getBlack());
			//v -join- v.grey, this will always form a 1-cycle
			if(!suppress){tempbuf.append("JOIN: "+ v.getBlack().getGene()+" to "+v.getGrey().getBlack().getGene()+"\n");}
			v.joinBlack(v.getGrey());
			this.greyVector.remove(this); //remove The 1-cycle formed from the DCJ
	...
	public void parseAndLoadGrey(String s,SB[] sbArray)
	{
		...
		int count=0;
		//use intArray[] to manipulate existing objects in sbArray[] (join grey)
		for(int i=0;i<intArray.length-1;i++)
		{
			sbArray[Math.abs(intArray[i])-1].setValueB(intArray[i]);
			sbArray[Math.abs(intArray[i+1])-1].setValueB(intArray[i+1]);
			sbArray[Math.abs(intArray[i])-1].getHeadB().joinGrey(sbArray[Math.abs(intArray[i+1])-1].getTailB());
			//sbArray[Math.abs(intArray[i])-1].getRight().joinGrey(sbArray[Math.abs(intArray[i+1])-1].getLeft());

		}//end for
		capVector.add(sbArray[Math.abs(intArray[0])-1].getTailB().cap(false));
		capVector.add(sbArray[Math.abs(intArray[intArray.length-1])-1].getHeadB().cap(false));
	}
	...
	public void parseAndLoadBlack(String s)
	{
		...
		{
			//CONNECT black lines 
			((SB)tempSbVector.elementAt(i)).getRight().joinBlack(((SB)tempSbVector.elementAt(i+1)).getLeft());
		}//end for
		//add A Caps to front and end of chromosome
		capVector.add((((SB)tempSbVector.firstElement()).getLeft()).cap(true));
		capVector.add((((SB)tempSbVector.lastElement()).getRight()).cap(true));

		this.sbVector.addAll(tempSbVector);
	}//end parseAndLoadBlack

	/**********************************************************
	 traverses the genome starting from caps from the cap vector
	 checks if the caps have been visited, if visited it passes it
	 otherwise it calls traversePath from class Vert. 
	 Then it closes the path from the start cap to the end cap.
	 At this point the genome data structure is complete
	 And lastly, it populates the grey vector by iterating through the
	 SB Vector and adding all grey lines.
	 ************************************************************/
	...
	public SB[] toSBArray()
	{
		...
		for(Enumeration e=this.sbVector.elements();e.hasMoreElements();)
		{
			...
			if(max<Math.abs(tempSB.getValueA())) max=Math.abs(tempSB.getValueA());
		}
	...
	public void loadGenome(StringTokenizer tokenA,StringTokenizer tokenB)
	{
		while(tokenA.hasMoreTokens())
		{
			...
			catch(Exception e){System.out.println("loadBlack error");System.out.println(e);}
		}//end while
		//this.printSBV();
		...
		while(tokenB.hasMoreTokens())
		{
			...
			catch(Exception e){System.out.println("load Grey error");System.out.println(e);}
		}//end while
	...
	public void printSBV()
	{
		...
		/****uncomment to print the caps****
		 Vert tempCap;
		 for(Enumeration e=this.capVector.elements();e.hasMoreElements();){
		 tempCap=(Vert)e.nextElement();
		 tempCap.printMe(); 
		 }//end for
		 ************************************/	
	}
	...
	public void printSBBlack()
	{
		...
		for(int i=0;i<this.capVector.size();i++)
		{
			...
			if(!v.getBlack().getSB().wasVisited()&&!v.getBlack().isCap())
			{
