...
public class IntervalSequenceTree
{
    ...
    public Iterator insert(Key val, long point)
    {
        ...
        else
        {
            ...
            new_gp.getLeft().getKey().cropEnd(ins_node.getLength() - iv_offset[0]);
            new_gp.getLeft().setLength(new_gp.getLeft().getKey().getLength());
            ...
            new_parent.setLength(new_node.getLength() + ins_node.getLength());
            new_parent.setSubtreeSize(2);

            new_gp.setLength(ins_node.getLength() + new_gp.getLeft().getLength());
    ...
    public long erase(long point, long length)
    {
        ...
        while (length > 0)
        {
            ...
            {
                // crop from end
                ins_node.getKey().cropEnd(ins_node.getLength() - iv_offset[0]);
                length -= ins_node.getLength() - iv_offset[0];
                //			checkTree( root );
                propogateChanges(ins_node, -(ins_node.getLength() - iv_offset[0]), 0);
                ins_node = increment(ins_node);
                iv_offset[0] = 0;
            }
            else
            {
                ...
                new_parent.getRight().setKey((Key) ins_node.getKey().copy());
                new_parent.getRight().setLength(ins_node.getLength() - length - iv_offset[0]);
                new_parent.getRight().getKey().cropStart(length + iv_offset[0]);
                new_parent.getLeft().getKey().cropEnd(ins_node.getLength() - iv_offset[0]);
    ...
    IstNode increment(IstNode x)
    {
        ...
        {
            if (x == x.getParent().getLeft() && x.getParent().getRight() != null)
            {
    ...
    IstNode decrement(IstNode x)
    {
        if (x != null)
        {
            ...
            while (x.getParent() != null)
            {
                if (x == x.getParent().getRight() && x.getParent().getLeft() != null)
                {
