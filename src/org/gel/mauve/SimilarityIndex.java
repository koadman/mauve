...
public class SimilarityIndex implements Serializable
{
    ...
    SimilarityIndex(Genome g, XMFAAlignment xmfa, BackboneList bb_list) throws IOException
    {
    ...
    void advanceIndex(String[] cols, int[] col_index, int seq)
    {
        while (true)
        {
            if (cols[seq].charAt(col_index[seq]) == '\r' || cols[seq].charAt(col_index[seq]) == '\n')
            {
                col_index[seq]++;
                continue;
            }
    ...
    void skipGapColumns(Genome g, Object[] cols, int[] col_index)
    {
        while (true)
        {
            ...
            if (((byte[]) cols[g.getSourceIndex()])[col_index[g.getSourceIndex()]] == '-')
            {
                continue;
            }
            break;
     ...
     */
    public void calculateIndex(Genome g, XMFAAlignment alignment, BackboneList bb_list)
    {
        ...
        for (int indexI = 0; indexI < index_size[0]; indexI++)
        {
            ...
            if (rend > buffer_right)
            {
                ...
                for (int entI = 0; entI < ent_count; entI++)
                {
                    ...
                    // tally up character counts, using backbone info if available
                    if( bb_list == null )
                    {
                		...
                		// first check whether we're still in range of the backbone
                    	// update the current bb segment as necessary
                        if( bb != null && buffer_left + entI > bb.getRightEnd(g))
                        {
                        	bb = bb_list.getBackbone(g, buffer_left + entI);
                        	if( bb == null )
                        		next_bb = bb_list.getNextBackbone(g, buffer_left + entI);
                        }else
                    	if( bb == null && next_bb != null && 
                            next_bb.getLeftEnd(g) <= buffer_left + entI && buffer_left + entI <= next_bb.getRightEnd(g))
                    	{
                        	bb = next_bb;
                        	next_bb = null;
                    	}
                    	...
                    	if( bb == null )
                    	{
                    		...
                    		chars[char_map['-']]+=cols.length-1;	// add gap for the rest
                    	}
                    	else
                    	{
                    ...
                    for (i = 0; i < 4; i++)
                    {
                        if (chars[i] == 0)
                            continue;
                        entropies[entI] -= frac((int) chars[i], char_types) * log_2((int) chars[i], char_types);
                    }
                    for (i = 0; i < chars[4]; i++)
                    {
                        entropies[entI] -= frac(1, char_types) * log_2(1, char_types);
                    }
    ...
    public byte getSimilarityByRange(long left, long right)
    {
        ...
        if (right_size > resolutions[0] && levelI > 0 )
            right_sim = getSimilarityByRange((lastI + 1) * resolutions[levelI], right);
        else
            right_sim = sim;

        // average the three together
        double asim_singh = left_size * left_sim + right_size * right_sim + sim * (range_size - left_size - right_size);
    ...
    double frac(int numerator, int denominator)
    {
        ...
        if (numerator < 1 || denominator < 1 || numerator > 127 || denominator > 127)
    ...
    public void calculateLog2Table()
    {
        ...
        for (double x = 1; x < 128; x++)
        {
            for (double y = 1; y < 128; y++)
            {
                logs[(int) x - 1][(int) y - 1] = Math.log(x / y) / Math.log(2d);
            }
        }
    }
    ...
    double log_2(int numerator, int denominator)
    {
        ...
        if (numerator > 127 || denominator > 127)
            return Math.log((double) numerator / (double) denominator) / Math.log(2d);
