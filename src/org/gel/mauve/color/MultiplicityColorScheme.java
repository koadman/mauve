...
public class MultiplicityColorScheme implements ColorScheme
{
    ...
    public void apply(BaseViewerModel model)
    {
        ...
        for (int matchI = 0; matchI < model.getMatchCount(); matchI++)
        {
            ...
            color_count -= 2; // assume that all matches have multiplicity >= 2
    ...
    private void initColorTable(BaseViewerModel model)
    {
        ...
        {
            throw new RuntimeException("Model must have at least one sequence.");
        }
        ...
        int cycles = (int) Math.ceil((double) model.getSequenceCount() / (double) max_m);

        for (int seqI = 0; seqI < model.getSequenceCount(); seqI++)
        {
            float hue = ((float) seqI / (float) max_m) % 1;
            // add the cycle increment
            hue += ((1d / (double) max_m) / (double) cycles) * Math.floor((double) seqI / (double) max_m);
            _matchColorTable.addElement(Color.getHSBColor(hue, MATCH_SAT, MATCH_BRIGHT));
        }
    }
