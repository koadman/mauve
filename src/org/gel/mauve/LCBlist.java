...
public class LCBlist
{
     ...
     * orientation
     */
    public static void computeLCBAdjacencies(LCB[] lcb_list, BaseViewerModel model)
    {
     ...
     * minimum weight.
     */
    public static void greedyBreakpointElimination(long targetMinWeight, LCB[] fullLCBList, Vector change_points, LcbViewerModel model)
    {
        ...
        while (currentMinWeight < targetMinWeight)
        {
            ...
            for (int seqI = 0; seqI < model.getSequenceCount(); seqI++)
            {
                ...
                if (left_adj != ENDPOINT && fullLCBList[left_adj].getRightAdjacency(g) != lcbI)
                {
                    throw new Error("Inconsistency in LCB list.");
                }
                if (right_adj != ENDPOINT && fullLCBList[right_adj].getLeftAdjacency(g) != lcbI)
                {
                    throw new Error("Inconsistency in LCB list.");
                }
            ...
            for (int seqI = 0; seqI < model.getSequenceCount(); seqI++)
            {
                ...
                if (left_adj != fullLCBList[left_adj].id || right_adj != fullLCBList[right_adj].id)
                {
                ...
                for (seqJ = 0; seqJ < model.getSequenceCount(); seqJ++)
                {
                    ...
                    boolean j_orientation = !fullLCBList[left_adj].getReverse(g2);
                    if (j_orientation == orientation && fullLCBList[left_adj].getRightAdjacency(g2) != right_adj)
                        break;
                    if (j_orientation != orientation && fullLCBList[left_adj].getLeftAdjacency(g2) != right_adj)
                        break;
                ...
                if (fullLCBList[right_adj].id == ENDPOINT || fullLCBList[right_adj].id == REMOVED)
                {
                ...
                for (seqJ = 0; seqJ < model.getSequenceCount(); seqJ++)
                {
                    ...
                    {
                        fullLCBList[left_adj].setRightEnd(g2, fullLCBList[right_adj].getRightEnd(g2));
                    ...
                    {
                        fullLCBList[left_adj].setLeftEnd(g2, fullLCBList[right_adj].getLeftEnd(g2));
                        ...
                        if (rl_adj != ENDPOINT)
                            fullLCBList[rl_adj].setRightAdjacency(g2, left_adj);
                    }
     ...
     */
    public static LCB[] filterLCBs(LCB[] interimList, BaseViewerModel model, Vector removed_lcbs, boolean updateIDsAndColors)
    {
        ...
        for (int lcbIndex = 0; lcbIndex < interimList.length; lcbIndex++)
        {
            ...
            if (currentID != ENDPOINT && currentID != REMOVED && interimList[currentID].id != currentID)
            {
                Stack visited = new Stack();
                visited.push(new Integer(lcbIndex));
                while (currentID != ENDPOINT && currentID != REMOVED && interimList[currentID].id != currentID)
                {
