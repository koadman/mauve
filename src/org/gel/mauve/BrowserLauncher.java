	...
	public static void openURL(String url) throws IOException {
		...
		switch (jvm) {
			...
			case OTHER:
				...
				if( !launch_success ){
					try {
						process = Runtime.getRuntime().exec(new String[] { "opera" , url });
						process.waitFor();
						launch_success = true;
					} catch (InterruptedException ie) {
						throw new IOException("InterruptedException while launching browser: " + ie.getMessage());
				...
				if( !launch_success ){
					try {
						process = Runtime.getRuntime().exec(new String[] { "konqueror" , url });
						process.waitFor();
						launch_success = true;
					} catch (InterruptedException ie) {
						throw new IOException("InterruptedException while launching browser: " + ie.getMessage());
				...
				if( !launch_success ){
					try {
						process = Runtime.getRuntime().exec(new String[] { "netscape" , url });
						process.waitFor();
						launch_success = true;
					} catch (InterruptedException ie) {
						throw new IOException("InterruptedException while launching browser: " + ie.getMessage());
