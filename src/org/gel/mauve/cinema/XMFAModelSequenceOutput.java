...
public class XMFAModelSequenceOutput extends CinemaGuiModule implements
		CinemaActionProvider, ParserExceptionHandler {
	...
	public void saveAlignment (final SequenceAlignment sa) {
		final JDialog dialog = new JDialog (getAlignmentFrame (),
				"Saving and reloading", true);
		JPanel panel = new JPanel ();
		panel.add (new JLabel ("Saving alignment"));
		dialog.setDefaultCloseOperation (JDialog.DO_NOTHING_ON_CLOSE);
		// FIXME: Why isn't this message appearing??!?!?!?!?
		dialog.getContentPane ().add (
				new JLabel ("Saving file and reloading viewer."));
		...
		dialog.addWindowListener (new WindowAdapter () {
			public void windowOpened (WindowEvent evt) {
				XMFASource xmfaSource = (XMFASource) sa.getIdentifier ()
						.getSource ();
				File tmp;
				try {
	...
	private void saveToFile (SequenceAlignment sa, File dest) {
		...
		try {
			...
			JOptionPane.showMessageDialog (null,
					"<html>There was a problem with saving the results: <p>"
							+ e.getMessage (), "Warning",
					JOptionPane.ERROR_MESSAGE);

			if (Debug.debug)
				Debug.both (this,
						"File Sequence Output: An IOException was found", e);
		}
	}
