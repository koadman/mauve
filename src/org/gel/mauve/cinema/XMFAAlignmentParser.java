...
public class XMFAAlignmentParser extends AbstractProteinAlignmentInputParser implements AlignmentOutputParser
{
    ...
    public SequenceAlignment parse(Identifier ident, Reader reader, ParserExceptionHandler eh) throws IOException
    {
        ...
        while (sequence != null)
        {
            String title = xmfaSource.getModel().getGenomeBySourceIndex(sourceIndex).getDisplayName();
            if (range != null)
            {
                title += " [" + range[0] + "-" + range[1] + "]";
            }
            
            GappedSequence gappedSequence = toSequence(sequence.toUpperCase(), "sequence " + sourceIndex, eh);
    	    try
            {
            ...
            gappedSequence = Sequences.getElementsAsGappedSequence(gappedSequence.getGappedSequenceAsElements(), ProteinSequenceType.getInstance(), new XMFASequenceIdentifier(title, range));
        ...
        return new DefaultSequenceAlignment(ar, ProteinSequenceType.getInstance(), insets.toArray(), ident);

    }

    public SequenceAlignment parse(Reader reader, ParserExceptionHandler eh) throws IOException
    {
    ...
    {
        // Assumption:  the alignments identifier will be changed to the target for output.
        ...
        try
        {
            ...
            for (int i = 1; i <= sa.getNumberSequences(); i++)
            {
                ...
                {
                    // Sequences in editor are indexed by 1, so we need to adjust.
