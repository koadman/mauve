...
public class XMFAAlignment implements Serializable
{
    ...
    /** 
     * Sets the file used as backing store for this alignment
     * Call this method to set the file after reading a serialized XMFAAlignment 
     * @param f	The xmfa file corresponding to this alignment
     */
    ...
    public XMFAAlignment(RandomAccessFile ir) throws java.io.IOException
    {
        ...
        newline_size = (int) (xmfa_file.getFilePointer() - first_line.length());
        xmfa_file.seek(0);
        ...
        while (true)
        {
            if (remaining_bytes == 0)
            {
                ...
                remaining_bytes = xmfa_file.read(buf, copy_len, buf.length - copy_len);
                cur_base -= copy_len;
                if (remaining_bytes <= 0)
                    break;
            ...
            case read_metadata:
                ...
                {
                    String cur_line = new String(buf, section_start, bufI - section_start);

                    // is this a sequence count specifier?
                    int sc_index = cur_line.indexOf("SequenceCount");
                    if (sc_index >= 0)
                    {
                        seq_count = Integer.parseInt(cur_line.substring(sc_index + 13).trim());
                    }
                    else
            ...
            case read_defline:
                if (buf[bufI] == '\r' || buf[bufI] == '\n')
                {
                    ...
                    int seq_num = -1;
                    java.util.StringTokenizer seq_num_strtok = new java.util.StringTokenizer(cur_line.substring(1, colon_pos));
                    if (seq_num_strtok.hasMoreTokens())
                    {
                        seq_num = Integer.parseInt(seq_num_strtok.nextToken()) - 1;
                    ...
                    long first = Long.parseLong(tmp_str.substring(0, dash_pos));
                    long second = Long.parseLong(tmp_str.substring(dash_pos + 1, space_pos));
                    ...
                    if (tmp_comments.size() == 0)
                    {
                        int strand_pos = 0;
                        if (!cur_reverse)
                            strand_pos = tmp_str.indexOf('+', space_pos + 1);
                        else
                            strand_pos = tmp_str.indexOf('-', space_pos + 1);

                        if (tmp_str.length() > strand_pos + 2)
                            // there's a name to parse
                            tmp_names.add(tmp_str.substring(strand_pos + 2));
                        else
            ...
            case read_comment:
                if (buf[bufI] == '\r' || buf[bufI] == '\n')
                {
                    ...
                    if (cur_comment == null)
                    {
                        cur_comment = new String(buf, section_start, bufI - section_start);
            ...
            case read_sequence:
                ...
                // set the sequence data file offset if this is the beginning of
                // the entry
                if (fk == null && gk == null)
                {
            ...
            case process_sequence:
                ...
                else if (buf[bufI] == '=')
                {
                    ...
                    int aligned_count = 0;
                    // only set values for sequence numbers that were actually
                    // part
                    // of this interval
                    for (int seq_numI = 0; seq_numI < seq_nums.size(); seq_numI++)
                    {
                        int seqI = ((Integer) seq_nums.elementAt(seq_numI)).intValue();
                        m.setStart(seqI, ((Long) lend.elementAt(seq_numI)).longValue());
                        if (m.getStart(seqI) != 0)
                            aligned_count++;
                        m.setLength(seqI, ((Long) rend.elementAt(seq_numI)).longValue());
                        m.setReverse(seqI, ((Boolean) reverse.elementAt(seq_numI)).booleanValue());
                        f_off_m.setStart(seqI, ((Long) f_offset.elementAt(seq_numI)).longValue());
                        f_off_m.setLength(seqI, ((Long) f_end_offset.elementAt(seq_numI)).longValue());
                    }
        ...
        for (int seqI = 0; seqI < seq_count; seqI++)
        {
            ...
            for (int ivI = 0; ivI < intervals.length; ivI++)
            {
                ...
                if (gis_tree[ivI][seqI] == null)
                {
                    ...
                    {
                        if (gis_tree[ivI][seqJ] != null && iv_length < gis_tree[ivI][seqJ].length())
                        {
    ...
    public Object[] getRange(Genome g, long lend, long rend)
    {
        ...
        while (cur_offset <= rend)
        {
            ...
            long read_size = intervals[ivI].getLength(g) - rend > 0 ? rend - cur_offset + 1 : intervals[ivI].getLength(g) - cur_offset + 1;
            while (read_size > 0)
            {
                ...
                long lcb_right_offset = intervals[ivI].getReverse(g) ? intervals[ivI].getLength(g) - intervals[ivI].getStart(g) - lcb_offset + 1 : lcb_offset + read_size;
                lcb_offset = intervals[ivI].getReverse(g) ? lcb_right_offset - read_size : lcb_offset;

                long fk_left_col = gis_tree[ivI][g.getSourceIndex()].seqPosToColumn(lcb_offset);
                long fk_right_col = gis_tree[ivI][g.getSourceIndex()].seqPosToColumn(lcb_right_offset);

                Object[] byte_bufs = new Object[seq_count];
                for (seqJ = 0; seqJ < seq_count; seqJ++)
                {
                    byte_bufs[seqJ] = readRawSequence(ivI, seqJ, fk_left_col, fk_right_col - fk_left_col);
                }
                ...
                for (seqJ = 0; seqJ < seq_count; seqJ++)
                {
                    byte[] tmp_buf = new byte[((byte[]) cols[seqJ]).length + ((byte[]) byte_bufs[seqJ]).length];
                    System.arraycopy((byte[]) cols[seqJ], 0, tmp_buf, 0, ((byte[]) cols[seqJ]).length);
                    System.arraycopy((byte[]) byte_bufs[seqJ], 0, tmp_buf, ((byte[]) cols[seqJ]).length, ((byte[]) byte_bufs[seqJ]).length);
                    cols[seqJ] = tmp_buf;
                }
            }
        }
    ...
    byte[] readRawSequence(int ivI, int seqI, long left_col, long length)
    {
        ...
        if (gis_tree[ivI][seqI].sequenceLength() <= left_col && gis_tree[ivI][seqI].getSeqLength(l_iter) == 0 && seq_off <= left_col)
        {
        ...
        if (gis_tree[ivI][seqI].getKey(l_iter) instanceof GapKey && gis_tree[ivI][seqI].getKey(r_iter) instanceof GapKey && gis_tree[ivI][seqI].getKey(l_iter) == gis_tree[ivI][seqI].getKey(r_iter))
        {
            byte[] bb = new byte[(int) length];
            for (int bbI = 0; bbI < bb.length; bbI++)
                bb[bbI] = (byte) '-';
        ...
        try
        {
            ...
            xmfa_file.read(byte_buf, (int) l_gaps, byte_buf.length - (int) l_gaps);
        }
        catch (IOException e)
        {
    ...
    int getLCB(Genome g, long position)
    {
        ...
        {
            if (intervals[ivI].getStart(g) <= position && position <= intervals[ivI].getLength(g))
        ...
        if (ivI == intervals.length)
            throw new ArrayIndexOutOfBoundsException("genome " + g + " position " + position);
        return ivI;
    }
    ...
    {
        return intervals[ivI].getReverse(g) ? intervals[ivI].getLength(g) - intervals[ivI].getStart(g) - position : position;
    }
