 ...
 * @author pinfield
 * 
 * A combination of alignment data, in the list of genomes, and viewing data, 
 * in the form of everything else.  This doesn't include LCBs or anything
 * XMFA-specific.
 * 
 * This is a test.
 * 
...
public class BaseViewerModel
{
    ...
    /**
     * Create a model with the specified source.  Note that this does
     * not load the model, use {@link org.gel.mauve.ModelBuilder#buildModel(File,ModelProgressListener)}
     * for reading files.
     * 
     * @param src file from which data came
     */
    ...
    /**
     * Set the number of genomes to be added to this model.  Only
     * used at construction-time, since this will clear out 
     * preexisting genome data.
     * 
     * @param sequenceCount
     ...
     * That is, suppose that the file contains the order A B C D, and the viewer shows
     * A B D C.  Then, getGenome(3) will return genome C.
     * 
     * @param viewIndex
    ...
    /**
     * Set the color scheme to be applied in the viewer, and apply it.  This will cause
     * the firing of {@link ModelListener@colorChanged(ModelEvent)} for any
     * registered listeners, if the scheme is different than the current one.
     * 
    ...
    /**
     * Invoke {@link HighlightListener.highlightChanged(ModelEvent)} on
     * this model's collection of HighlightListeners.
     */
    protected void fireHighlightEvent() {
        Object[] listeners = listenerList.getListenerList();
        for (int i = listeners.length-2; i>=0; i-=2) {
            if (listeners[i]==HighlightListener.class) {
                ((HighlightListener)listeners[i+1]).highlightChanged(modelEvent);
            }
        }
    }

    /**
     * Invoke {@link ModelListener.colorChanged(ModelEvent)} on
     * this model's collection of ModelListeners.
     */
    ...
    /**
     * Invoke {@link ModelListener.weightChanged(ModelEvent)} on
     * this model's collection of ModelListeners.
     */
    ...
    /**
     * Invoke {@link ModelListener.drawingSettingsChanged(ModelEvent)} on
     * this model's collection of ModelListeners.
     */
    protected void fireDrawingSettingsEvent() {
        ...
        for (int i = listeners.length-2; i>=0; i-=2) {
            if (listeners[i]==ModelListener.class) {
                ((ModelListener)listeners[i+1]).drawingSettingsChanged(modelEvent);
            }
        }
    }
    
    /**
     * Invoke {@link ModelListener.modeChanged(ModelEvent)} on
     * this model's collection of ModelListeners.
     */
    ...
    /**
     * Invoke {@link ModelListener.viewableRangeChanged(ModelEvent)} on
     * this model's collection of ModelListeners.
     */
    protected void fireViewableRangeEvent() {
        Object[] listeners = listenerList.getListenerList();
        for (int i = listeners.length-2; i>=0; i-=2) {
            if (listeners[i]==ModelListener.class) {
                ((ModelListener)listeners[i+1]).viewableRangeChanged(modelEvent);
            }
        }
    }
    
    /**
     * Invoke {@link ModelListener.viewableRangeChangeStart(ModelEvent)} on
     * this model's collection of ModelListeners.
     */
    protected void fireViewableRangeStartEvent() {
        ...
        for (int i = listeners.length-2; i>=0; i-=2) {
            if (listeners[i]==ModelListener.class) {
                ((ModelListener)listeners[i+1]).viewableRangeChangeStart(modelEvent);
            }
        }
    }

    /**
     * Invoke {@link ModelListener.viewableRangeChangeEnd(ModelEvent)} on
     * this model's collection of ModelListeners.
     */
    protected void fireViewableRangeEndEvent() {
        ...
        for (int i = listeners.length-2; i>=0; i-=2) {
            if (listeners[i]==ModelListener.class) {
                ((ModelListener)listeners[i+1]).viewableRangeChangeEnd(modelEvent);
            }
        }
    }
    
    /**
     * Invoke {@link ModelListener.genomesReordered(ModelEvent)} on
     * this model's collection of ModelListeners.
     */
    protected void fireReorderGenomeEvent() {
        ...
        for (int i = listeners.length-2; i>=0; i-=2) {
            if (listeners[i]==ModelListener.class) {
                ((ModelListener)listeners[i+1]).genomesReordered(modelEvent);
            }
        }
    }

    /**
     * Invoke {@link ModelListener.referenceChanged(ModelEvent)} on
     * this model's collection of ModelListeners.
     */
    protected void fireReferenceChangedEvent() {
        ...
        for (int i = listeners.length-2; i>=0; i-=2) {
            if (listeners[i]==ModelListener.class) {
                ((ModelListener)listeners[i+1]).referenceChanged(modelEvent);
            }
        }
    }
    ...
    /**
     * Set the current drawing mode for the application.  If this
     * results in a change of mode, then {@link ModelListener#modeChanged(ModelEvent)}
     * will be fired for all of the model's ModelListeners.
     * 
     * @param mode
     */
    ...
    /**
     * Reorder genomes according to given permutation array.  For example, if
     * the genomes are being viewed in the order A B C D, and the array is 1 0 3 2, 
     * then the new viewing order will be B A D C.  The permutation array is applied
     * to the genomes in viewing order, not source order.
     * 
     * This method will cause the firing of {@link ModelListener#genomesReordered(ModelEvent)}
     * for all registered ModelListeners.
     * 
     * @param new_order permutation array
     */
     ...
     */
    public void getMatchRange(Genome g, long start_coord, long end_coord, int[] match_range)
    {
        ...
        int match_startI = Collections.binarySearch(sortedMatches, f_match, matchComparator);
        match_startI = match_startI >= 0 ? match_startI : -match_startI - 1;

        f_match.setStart(g, end_coord + 1);
        int match_endI = Collections.binarySearch(sortedMatches, f_match, matchComparator);
        ...
        if (match_startI > 0)
        {
            ...
            if (prev_match.getStart(g) + prev_match.getLength(g) > start_coord && prev_match.getStart(g) != Match.NO_MATCH)
                match_startI--;
        }
    ...
    {
        if (highBoxes.isEmpty()) return null;
    ...
    public void alignView(long[] align_coords, Genome selected)
    {
        ...
        long start_offset = align_coords[selected.getSourceIndex()] - selected.getViewStart();
        for (int seqI = 0; seqI < getSequenceCount(); seqI++)
        {
            if (seqI != selected.getSourceIndex() && align_coords[seqI] != Match.NO_MATCH)
            {
    ...
    private void zoomAndMove(Genome g, int zoom_percent, long offset)
    {
        ...
        else if (zoom_percent != 100)
        {
            ...
            long new_view_start = g.getViewStart() + ((g.getViewLength() - new_view_length) / 2);
