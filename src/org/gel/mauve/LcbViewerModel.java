...
public class LcbViewerModel extends BaseViewerModel
{
    ...
    public void updateHighlight(Genome g, long leftCoordinate, long rightCoordinate)
    {
    ...
    public LCB getVisibleRightNeighbor(LCB lcb, Genome g, long pos)
    {
        ...
        while (rightIndex >= 0 && pos > visibleLcbList[rightIndex].getRightEnd(g))
        {
            ...
            for (seqI = 0; seqI < getSequenceCount(); seqI++)
            {
                ...
                if (lcb.getLeftEnd(g) == Match.NO_MATCH)
                {
                    if (m.getStart(g) != Match.NO_MATCH)
                    {
                        ...
                        {
                            lcb.setLeftEnd(g, m.getStart(g) + m.getLength(g));
                        }
                    }
                }
                else
        ...
        for (matchI = match_end; matchI > match_start; matchI--)
        {
            ...
            for (seqI = 0; seqI < getSequenceCount(); seqI++)
            {
                ...
                if (lcb.getRightEnd(g) == Match.NO_MATCH)
                {
                    ...
                    {
                        if (!m.getReverse(g))
                            lcb.setRightEnd(g, m.getStart(g) + m.getLength(g));
                        else
...
/*        for (int matchI = 0; matchI < getMatchCount(); matchI++)
        {
        	Match m = getMatch(matchI);
        	for (int genomeI = 0; genomeI < getSequenceCount(); genomeI++)
        	{
        		Genome g = getGenomeBySourceIndex(genomeI);
        		if( m.lcb != LCBlist.ENDPOINT && m.lcb != LCBlist.REMOVED )
        			m.setReverse(g, fullLcbList[m.lcb].getReverse(g));
        	}
        }
*/    
    }
    ...
    public long[] getHighlightArray(Genome g)
    {
        ...
        for (int matchI = match_range[0]; matchI < match_range[1]; matchI++)
        {
            ...
            if(m.getReverse(high_g) == m.getReverse(g))
            	g_highlights[highI] = m.getStart(g) + (high_c - m.getStart(high_g));
            else
            	g_highlights[highI] = m.getStart(g) + m.getLength(g) - (high_c - m.getStart(high_g));
            highI++;
        }
        return g_highlights;
    }
    ...
    public void alignView(Genome g, long left, long right)
    {
        ...
        for(int gI = 0; gI < genomes.length; gI++)
        {
            ...
            if(m.getReverse(cur_g) == m.getReverse(g))
            	...
            	coords[gI] = m.getStart(cur_g) + m.getLength(cur_g) - (right - m.getStart(g));
        }
        alignView(coords, g);
    }
    ...
    public void updateLCBweight(int min_weight, boolean temporary)
    {
        ...
        LCBlist.greedyBreakpointElimination(min_weight, visibleLcbList, null, this);

        if (temporary)
        {
            visibleLcbList = LCBlist.filterLCBs(visibleLcbList, this, null, false);
        }
        else
        {
            for (int matchI = 0; matchI < getMatchCount(); matchI++)
            ...
            visibleLcbList = LCBlist.filterLCBs(visibleLcbList, this, del_vec, true);
    ...
    public void sanityCheck()
    {
        for (int i = 0; i < getVisibleLcbCount(); i++)
        {
            ...
            if (lcb.id != LCBlist.REMOVED)
            {
                if (lcb.id != i)
                {
                    throw new Error("LCB with incorrect id.  Expected " + i + " but was " + lcb.id);
                }
                ...
                for (int j = 0; j < getSequenceCount(); j++)
                {
                    ...
                    if (lcb.getLeftAdjacency(g) != LCBlist.ENDPOINT && lcb.getLeftAdjacency(g) != LCBlist.REMOVED)
                    {
                    ...
                    if (lcb.getRightAdjacency(g) != LCBlist.ENDPOINT && lcb.getRightAdjacency(g) != LCBlist.REMOVED)
                    {
    ...
    public void initModelLCBs()
    {
        ...
        {
            if (getFullLcbList()[lcbI].weight < lcb_minimum_weight || lcb_minimum_weight == -1)
        ...
        {
	        try{
		        LCBlist.greedyBreakpointElimination(Long.MAX_VALUE, tmp_lcb_list, lcb_change_points, this);
    ...
    public void launchDCJ()
    {
    	...
    	{
    		int cur_count = this.getGenomeByViewingIndex(seqI).getChromosomes().size();
    		max_chr_count = cur_count > max_chr_count ? cur_count : max_chr_count;
    	}
    	...
    	for( int seqI = 0; seqI < getSequenceCount(); seqI++ )
    	{
    		...
    		{
    			chr_lens[seqI][chrI] = ((Chromosome)chromo.get(chrI)).getEnd();
    		}
    	}
    	...
    	for( int seqI = 0; seqI < getSequenceCount(); seqI++ )
    	{
    		...
    		while( adjI != LCBlist.ENDPOINT && adjI != LCBlist.REMOVED && adjI < visibleLcbList.length )
    		{
    ...
    public void launchGrimmMGR()
    {
    	...
    	for( int seqI = 0; seqI < getSequenceCount(); seqI++ )
    	{
    		...
    		int adjI = leftmost_lcb;
    		int cur_chromosome = 0;
    		all_circular = all_circular && ((Chromosome)chromo.get(cur_chromosome)).getCircular();
    		while( adjI != LCBlist.ENDPOINT && adjI != LCBlist.REMOVED && adjI < visibleLcbList.length )
    		{
    			if( visibleLcbList[ adjI ].getLeftEnd(g) > chr_lens[seqI][cur_chromosome] )
    			{
    	...
    	if( url_data.length() < 2000 )
    	{
	    	...
	    	try{
	    		...
	    		bw.write("<form method=POST action=\"http://nbcr.sdsc.edu/GRIMM/grimm.cgi\">\n");
	    		bw.write("<input type=\"hidden\" name=\"ismult\" value=\"1\">\n");
	    		bw.write("<input type=\"hidden\" name=\"ngenomewins\" value=\"" + getSequenceCount() + "\">\n");
	    		for( int seqI = 0; seqI < getSequenceCount(); seqI++ )
	    		{
		    		bw.write("<input type=\"hidden\" name=\"genome" + (seqI + 1 ) + "\" value=\"" + per_genome[seqI] + "\">\n");
	    		}
	        	if( single_chromosome && all_circular )
		    		bw.write("<input type=\"hidden\" name=\"nchromosomes\" value=\"circular\">\n");
	        	else
		    		bw.write("<input type=\"hidden\" name=\"nchromosomes\" value=\"multichromosomal\">\n");
	    		bw.write("<input type=\"hidden\" name=\"signedperm\" value=\"signed\">\n");
	    		bw.write("<input type=\"submit\" name=\"action\" value=\"run\"");
	    		bw.write("</form>\n</body>\n</html>");
	    		bw.close();
		    	BrowserLauncher.openURL("file://" + tmp_form.getCanonicalPath());
	    	}catch(IOException ioe)
	    	{
	    		ioe.printStackTrace();
	    	}
