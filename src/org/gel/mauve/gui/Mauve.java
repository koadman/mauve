...
public class Mauve
{
    ...
    private Vector frames; // List of open frames
    ...
    public void init(String filename)
    {
        ...
        }else	// this looks like a URL, try to load it
        	try{
    ...
    public synchronized void init()
    {
        ...
        try
        {
            InputStream props_stream = MauveFrame.class.getResourceAsStream("/version.properties");
...
//        mauve_splash.setVisible(false);
    ...
    public void checkForUpdates()
    {
        try
        {
            ...
            if (os_type.startsWith("Windows"))
                ...
                os_suffix = "mac";
            URL hurl = new URL("http://gel.ahabs.wisc.edu/mauve/downloads/latest." + os_suffix);
            ...
            if (local_version < latest_version)
            {
                ...
                {
                    JOptionPane.showMessageDialog(null, "<html>An updated version of Mauve is available.<br>See the Mauve web site at <a href=\"http://gel.ahabs.wisc.edu/mauve\">http://gel.ahabs.wisc.edu/mauve</a> for more details...", "Updated Mauve available", JOptionPane.INFORMATION_MESSAGE);
                    return;
                }
    ...
    private boolean hasRequiredJVM()
    {
        ...
        if (jvm_version.charAt(0) == '1')
        {
            if (minor_version < 4)
            {
                MyConsole.err().println("Sorry, Mauve requires at least Java version 1.4 to operate correctly");
                return false;
            }
        }
        return true;
    ...
    public synchronized void setFocus(String alignID, String sequenceID, long start, long end, String auth_token)
    {
        URL alignURL;
        try
        {
        ...
        while (it.hasNext())
        {
            ...
            {
                frame.model.setFocus(sequenceID, start, end);
                frame.toFront();
                return;
            }
        }
        ...
        Thread t = new Thread(new FrameLoader(frame, alignURL, sequenceID, start, end, auth_token));
        t.start();
